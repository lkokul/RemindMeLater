...
router.post('/', (req, res) => {
  const { sagaId, title, type, description, rating, status, genres, progressCurrent, progressTotal, progressUnit, ownedCount, ownedTotal } = req.body || {};
  ...
  const info = db
    .prepare(`
      INSERT INTO lecturas_items
        (saga_id, title, type, description, rating, status, genres, progress_current, progress_total, progress_unit, owned_count, owned_total, position)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `)
    .run(
      sagaId,
...
router.put('/:id', (req, res) => {
  ...
  const { title, type, description, rating, status, genres, progressCurrent, progressTotal, progressUnit, ownedCount, ownedTotal } = req.body || {};
  ...
  db.prepare(`
    UPDATE lecturas_items SET
      ...
      owned_count = ?, owned_total = ?, updated_at = datetime('now')
    WHERE id = ?
  `).run(
