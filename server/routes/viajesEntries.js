...
// /api/notes/images. Sirve tanto fotos sueltas como fotos de ticket de
// un movimiento -- ambas viven en el mismo DATA_DIR/viajes-photos.
...
router.post('/:id/movements', requireDeviceOrTrusted, (req, res) => {
  const entry = db.prepare('SELECT * FROM viajes_entries WHERE id = ?').get(req.params.id);
  if (!entry) return res.status(404).json({ error: 'not_found' });
...
router.put('/movements/:id', requireDeviceOrTrusted, (req, res) => {
...
router.delete('/movements/:id', requireDeviceOrTrusted, (req, res) => {
  const existing = db.prepare('SELECT * FROM viajes_entry_movements WHERE id = ?').get(req.params.id);
  if (!existing) return res.status(404).json({ error: 'not_found' });
...
router.post('/movements/:id/link-finanzas', requireDeviceOrTrusted, (req, res) => {
...
// enlazo por error.
router.delete('/movements/:id/link-finanzas', requireDeviceOrTrusted, (req, res) => {
  const mv = db.prepare('SELECT * FROM viajes_entry_movements WHERE id = ?').get(req.params.id);
  if (!mv) return res.status(404).json({ error: 'not_found' });
