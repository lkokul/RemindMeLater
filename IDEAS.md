# RemindMeLater — Ideario completo del proyecto
...
(rama `movil-ui`): la mayoría ya existen ahí, otras se quitaron por el
camino, y otras están solo apuntadas. Pensado como catálogo de
inspiración para desarrollar el proyecto completo de escritorio.

**Cómo encaja con el resto de documentos de esta rama**:
`README.md` es la documentación de usuario, `CLAUDE.md` explica cómo
está montado el código HOY, `PROYECTO-ESCRITORIO.md` es el plan de
trabajo de escritorio (qué queda por hacer y por qué se decidió cada
cosa), y ESTE archivo es el ideario completo del proyecto entero visto
desde el lado móvil — qué se llegó a construir allí y qué ideas quedan
sueltas, para robar de aquí lo que convenga.
