// entretenimientoSagas — portado de server/routes/entretenimientoSagas.js.
//
...
(function () {
  const db = localDb;
  // routes/entretenimientoSagas.js — sagas de la extension Entretenimiento (ver
  ...
  function serialize(row) {
    const { count } = db.prepare('SELECT COUNT(*) as count FROM entretenimiento_items WHERE saga_id = ?').get(row.id);
    const types = db.prepare('SELECT DISTINCT type FROM entretenimiento_items WHERE saga_id = ? ORDER BY type').all(row.id).map((t) => t.type);
    return {
      id: row.id,
      name: row.name,
  ...
  router.get('/', (req, res) => {
    const rows = db.prepare('SELECT * FROM entretenimiento_sagas ORDER BY name COLLATE NOCASE ASC').all();
  ...
  router.post('/', (req, res) => {
    ...
    const info = db
      .prepare('INSERT INTO entretenimiento_sagas (name, description) VALUES (?, ?)')
      .run(name.trim(), description && description.trim() ? description.trim() : null);

    const row = db.prepare('SELECT * FROM entretenimiento_sagas WHERE id = ?').get(info.lastInsertRowid);
    res.status(201).json(serialize(row));
  });

  router.put('/:id', (req, res) => {
    const existing = db.prepare('SELECT * FROM entretenimiento_sagas WHERE id = ?').get(req.params.id);
    if (!existing) return res.status(404).json({ error: 'not_found' });

    const { name, description } = req.body || {};
    db.prepare("UPDATE entretenimiento_sagas SET name = ?, description = ?, updated_at = datetime('now') WHERE id = ?").run(
    ...
    const row = db.prepare('SELECT * FROM entretenimiento_sagas WHERE id = ?').get(req.params.id);
  ...
  mountLocalRouter('/api/entretenimiento-sagas', router);
