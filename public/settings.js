...
// para color, icono, selector, fecha, insertar-tabla, estilo de
// parrafo y color de resaltado, asi no hay que repetir esta logica en
// cada widget).
document.addEventListener('click', (e) => {
  if (e.target.closest('.color-popover, .icon-popover, .select-popover, .date-popover, .table-insert-popover, .paragraph-style-popover, .highlight-color-popover, .color-swatch-btn, .icon-swatch-btn, .select-field-trigger, .date-field-trigger, #note-table-insert-btn, #note-paragraph-style-btn, #note-highlight-btn')) return;
  closeAllPopovers();
});
...
document.getElementById('btn-proyectos-settings').addEventListener('click', openSettingsModal);
...
// --- Importar / exportar temas (a un archivo .json, para guardarlos
// fuera de la app o pasarlos a otra instalacion) ---
...
// Al cargar la app: averigua que tema tenemos activo segun la base de
// datos (que es la fuente de verdad) y lo aplica. Mientras tanto, el
// script del <head> ya habra pintado lo que hubiera en cache para evitar
// el parpadeo de arrancar con los colores por defecto.
async function syncActiveTheme() {
  try {
    const [themes, mine] = await Promise.all([api('/api/themes'), api('/api/themes/selection/mine')]);
    themeLibrary = themes;
    ...
    // Si algo falla, nos quedamos con lo que hubiera en cache local.
  }
}
syncActiveTheme();
...
// Este ajuste controla el aviso que saca la propia ventana mientras la
// app esta abierta. El aviso "de verdad" del sistema, que salta aunque
// tengas la ventana minimizada, lo dispara core/reminderChecker.js desde
// el proceso principal y no depende de este permiso.
