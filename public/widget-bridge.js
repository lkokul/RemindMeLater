...
async function actualizarWidgetDelDia() {
  ...
  if (!plugin) return;
  // EL await NO ES OPCIONAL: construirResumenDelDia() es asincrona, asi
  // que JSON.stringify() sobre la llamada serializa la PROMESA, o sea
  // "{}", y el widget se queda vacio para siempre sin dar ningun error.
  // Estaba asi en esta rama desde que se quitaron los avisos de
  // desarrollador: al borrar la linea del diagnostico se fue tambien el
  // await, y como todo esto es silencioso a proposito, no se noto.
