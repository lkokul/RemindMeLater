...
const state = {
  ...
  upcomingReminders: [], // ultima lista de "proximos recordatorios"
...
// ---------------------------------------------------------------------
// Capa de datos: como le pide la app las cosas a la base de datos.
//
// Hasta la version 0.33 esto era una peticion HTTP de verdad (fetch a
// http://localhost:3000) contra un servidor Express, porque el mismo
// servidor atendia tambien al movil por wifi. Ya no hay servidor: la
// ventana y la base de datos viven en el mismo programa y hablan por el
// canal interno de Electron (ver electron/preload.js y electron/ipc.js).
//
// Lo que NO cambio: se sigue pidiendo cada cosa por su ruta de siempre
// ('/api/events?from=...'), y esta funcion sigue devolviendo el objeto ya
// desempaquetado o lanzando un error con su mensaje. Por eso los ~165
// sitios de este archivo que llaman a api() no tuvieron que tocarse.
// ---------------------------------------------------------------------
async function api(path, options = {}) {
  let body = options.body ?? null;

  // Un File/Blob (subir una imagen a una nota, una foto a Viajes) no
  // puede viajar por el canal de Electron tal cual, hay que convertirlo
  // antes a ArrayBuffer. Lo demas ya viene como texto JSON.
  if (body instanceof Blob) body = await body.arrayBuffer();

  const { status, body: data } = await window.electronAPI.api(path, {
...
// ---------------------------------------------------------------------
// Aviso de version nueva EN GITHUB: se pregunta si hay algo mas nuevo
// que lo que tienes instalado (ver core/routes/update.js, que reutiliza
// el git ya configurado en este ordenador). "No para esta version" se
// recuerda en localStorage: la siguiente version SI volvera a avisar.
// ---------------------------------------------------------------------
...
// GET /api/update/info en core/routes/update.js. Es solo lectura y no
// habla con GitHub (a diferencia de checkForNewRelease), asi que
...
async function refreshVersionInfo() {
  const boxes = [document.getElementById('settings-version-info')].filter(Boolean);
...
// proceso que sigue corriendo tiene el viejo cargado en memoria — hay
// que reiniciar de verdad para que se note. La app se reinicia sola
// (ver relaunchApp en electron/preload.js).
...
async function init() {
  ...
  // esta visible, así que cuanto mas tarde se llame a esto, mas se nota
  // el "flashazo" del calendario antes de taparlo. Moverlo aqui
  ...
  renderNotesView();
  // Los recordatorios se siguen refrescando solos cada 30s porque su
  // contenido depende de la HORA (algo que era "dentro de 10 min" pasa a
  // ser "ahora"), y porque el aviso de escritorio marca el recordatorio
  // como ya disparado desde el proceso principal (ver
  // core/reminderChecker.js). Tareas y notas ya no hace falta refrescarlas
  // por su cuenta: cuando existia el movil, otro dispositivo podia
  // cambiarlas por detras; ahora nadie mas escribe en la base de datos.
...
// se repite muy de tarde en tarde — cada 6 horas basta para
