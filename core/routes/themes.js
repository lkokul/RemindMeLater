// routes/themes.js — biblioteca de temas de estilo y cual esta activo.
//
// Idea clave para entender este archivo: un tema se GUARDA en la tabla
// `themes` (la biblioteca: todos los que tienes creados), y aparte se
// recuerda cual de ellos estas usando ahora mismo, en `app_settings`.
// Son dos cosas distintas a proposito — borrar un tema de la biblioteca
// no es lo mismo que dejar de usarlo.
...
router.delete('/:id', (req, res) => {
  // Igual que al borrar un grupo: si el tema borrado era el que estaba
  // puesto, no se queda nada "roto" — simplemente deja de haber tema
  // elegido hasta que elijas otro.
...
// Que tema tiene puesto este ordenador. Antes esto devolvia una LISTA
// (el ordenador mas cada movil emparejado, para la pantalla de "copiar
// el estilo de otro dispositivo conectado"); ahora la app de escritorio
// es la unica que hay, asi que solo queda una respuesta posible y se
// devuelve directamente en vez de una lista de un solo elemento.
router.get('/selection/mine', (req, res) => {
  const setting = db.prepare("SELECT value FROM app_settings WHERE key = 'host_active_theme_id'").get();
  const themeId = setting && setting.value !== null ? Number(setting.value) : null;
  const theme = themeId ? db.prepare('SELECT name FROM themes WHERE id = ?').get(themeId) : null;
  res.json({ themeId: theme ? themeId : null, themeName: theme ? theme.name : null });
});

// Aplica un tema de la biblioteca a este ordenador.
